...
{
    /// <summary>
    /// Bordro / huzur hakkı hesaplama uçları. Ekranı Hesaplamalar > Bordro Hesaplaması.
    ///
    /// Eskiden <c>api/public/payroll</c> rotasında ve <c>[AllowAnonymous]</c> idi: sayfa
    /// girişin dışında açılıyordu. Servistte global bir yetki politikası olmadığı için
    /// (<c>AddAuthorization()</c> çıplak) korumayı controller'ın kendi <c>[Authorize]</c>
    /// bayrağı sağlar — bu yüzden bayrak sadece silinmedi, yerine açıkça kondu.
    /// Rota da diğer catalog controller'larıyla aynı önekle taşındı ki gateway'de ayrı
    /// (yetkisiz) bir rotaya ihtiyaç kalmasın (KARARLAR §78).
    /// </summary>
    [ApiController]
    [Route("api/catalog/payroll")]
    [Authorize]
